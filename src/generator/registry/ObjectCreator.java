	...
	private static void createItem(String[] data)
	{
		try
		{
			...
			for (int i = 2; i < data.length; i++)
			{
				...
				if (property.startsWith("durability=")) item.setDamage(Integer.parseInt(property.substring("durability=".length())));
	...
	private static void createItemFromBlock(String[] data)
	{
		try
		{
			...
			for (int i = 2; i < data.length; i++)
			{
				...
				if (property.startsWith("item_damage=")) item.setDamage(Integer.parseInt(property.substring("damage=".length())));
				if (property.startsWith("item_texture=")) item.setTextureType(Integer.parseInt(property.substring("texture=".length())));
				if (property.startsWith("item_lang=")) item.setLangType(property.substring("lang=".length()));
			}
