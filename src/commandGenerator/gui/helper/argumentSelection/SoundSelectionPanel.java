...
public class SoundSelectionPanel extends HelperPanel implements IBox
{
	...
	{
		super(Constants.PANELID_SOUND, title);
	}
	...
	{
		entryVolume = new CEntry(Constants.DATAID_NONE, "GUI:playsound.volume", "1");
		entryVolume.setEnabledContent(false);
		entryPitch = new CEntry(Constants.DATAID_NONE, "GUI:playsound.pitch", "1");
		entryPitch.setEnabledContent(false);
		entryVolumeMin = new CEntry(Constants.DATAID_NONE, "GUI:playsound.volume_min", "1");
		entryVolumeMin.setEnabledContent(false);

		ObjectBase[] list = Registerer.getObjectList(Constants.OBJECT_SOUND);
		buttonHelpSound = new HelpButton(Lang.get("HELP:sound." + list[0].getId()), list[0].getId());

		checkboxOptions = new CCheckBox(Constants.DATAID_NONE, "GUI:playsound.advanced");

		comboboxSound = new CComboBox(Constants.DATAID_NONE, "GUI:playsound.choose", list, this);
	}
	...
	public void setupFrom(Map<String, Object> data)
	{
		...
		checkboxOptions.setSelected((boolean) data.get(Constants.DATAID_SOUND_OPTIONS));
		entryVolume.setEnabledContent(checkboxOptions.isSelected());
		entryPitch.setEnabledContent(checkboxOptions.isSelected());
		entryVolumeMin.setEnabledContent(checkboxOptions.isSelected());

		if (data.get(Constants.DATAID_SOUND_VOL) != null) entryVolume.setTextField((String) data.get(Constants.DATAID_SOUND_VOL));
		if (data.get(Constants.DATAID_SOUND_PITCH) != null) entryPitch.setTextField((String) data.get(Constants.DATAID_SOUND_PITCH));
		if (data.get(Constants.DATAID_SOUND_VOLMIN) != null) entryVolumeMin.setTextField((String) data.get(Constants.DATAID_SOUND_VOLMIN));
	}
