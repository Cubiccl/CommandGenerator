...
public class ListSelectionPanel extends HelperPanel
{
	...
	{
		super(Constants.DATAID_NONE, id, type, list);
	}
	...
	private void add()
	{
		switch (type)
		{
			...
			case Constants.OBJECT_EFFECT:
				EffectSelectionPanel panelEf = new EffectSelectionPanel(Constants.DATAID_NONE, "GENERAL:effect");
			...
			case Constants.OBJECT_ENCHANT:
				EnchantSelectionPanel panelEn = new EnchantSelectionPanel(Constants.DATAID_NONE, "GENERAL:enchant", false);
			...
			case Constants.OBJECT_ITEM:
				...
				for (int i = 0; i < items.length; i++)
					items[i] = (Item) list[i];
				ItemSelectionPanel panelI = new ItemSelectionPanel(Constants.DATAID_NONE, "GENERAL:item", items, true, true);
	...
	private void edit()
	{
		...
		switch (type)
		{
			case Constants.OBJECT_ATTRIBUTE:
				AttributeSelectionPanel panelA = new AttributeSelectionPanel();
				map.put(Constants.PANELID_OPTIONS, objects.get(nbr));
			...
			case Constants.OBJECT_EFFECT:
				EffectSelectionPanel panelEf = new EffectSelectionPanel(Constants.PANELID_EFFECT, "GENERAL:effect");
				map.put(Constants.PANELID_EFFECT, objects.get(nbr));
			...
			case Constants.OBJECT_ENCHANT:
				EnchantSelectionPanel panelEn = new EnchantSelectionPanel(Constants.PANELID_ENCHANT, "GENERAL:enchant", false);
				map.put(Constants.PANELID_ENCHANT, objects.get(nbr));
			...
			case Constants.OBJECT_ITEM:
				...
				for (int i = 0; i < items.length; i++)
					items[i] = (Item) list[i];
				ItemSelectionPanel panelI = new ItemSelectionPanel(Constants.PANELID_ITEM, "GENERAL:item", items, true, true);
				ItemStack stack = (ItemStack) objects.get(nbr);
				map.put(Constants.PANELID_ITEM, ItemStack.generateFrom(stack.getItem().getId(), stack.getDamage(), stack.getCount(), stack.getTag().getValue(), stack.getSlot()));
